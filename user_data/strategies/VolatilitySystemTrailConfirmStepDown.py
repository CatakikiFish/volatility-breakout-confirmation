...
# isort: skip_file
# =============================================================================
# VolatilitySystemTrailConfirmStepDown：探索性分档跟踪（非已证实优势）
#
# 复制自冻结快照 confirm_v1_hyperopt（trail_atr_mult=4.5）。
# Confirm V1、confirm_v1_hyperopt、同名 JSON 与 freqtrade_confirm_fwd 一律不动。
# 只增加一次切换：2R 后仍用 4.5×ATR；最大浮盈首次达到 6R 后永久收紧为 3.0×ATR。
# trail_tightened 写入 custom_data，加仓重算均价/R 后也不得从 3.0 放宽回 4.5。
# 本轮不搜索 6R 或 3.0。
# =============================================================================
...
class VolatilitySystemTrailConfirmStepDown(IStrategy):
    """冻结 4.5 规则 + 6R 后永久收紧为 3.0×ATR。"""
    ...
    def _trail_stop_price(self, trade: Trade, r_dist: float, atr_raw: float) -> float:
        tightened = bool(
            trade.get_custom_data(key="trail_tightened", default=False)
        )
        ...
        if trade.is_short:
            ...
            mfe_r = (open_rate - extreme) / r_dist
        else:
            extreme = float(trade.max_rate or open_rate)
            mfe_r = (extreme - open_rate) / r_dist

        if not tightened and mfe_r >= self.trail_tighten_r:
            trade.set_custom_data(key="trail_tightened", value=True)
            tightened = True

        trail_mult = (
            self.trail_atr_mult_tight
            if tightened
            else self.trail_atr_mult
        )

        if trade.is_short:
            atr_stop = extreme + trail_mult * atr_raw
            lock_stop = open_rate - 0.5 * r_dist
            stop_price = min(lock_stop, atr_stop)
        else:
            atr_stop = extreme - trail_mult * atr_raw
            lock_stop = open_rate + 0.5 * r_dist
            stop_price = max(lock_stop, atr_stop)
        return stop_price
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        最大浮盈首次达到 6R 后永久改用 3.0×ATR。签名保留 after_fill，加仓后允许按新均价重算 R。"""
