...
# isort: skip_file
# =============================================================================
# RESEARCH — Confirm V3.1 锚点审计（仅审计，不改 V3.1）
#
# 母本：user_data/strategies/confirm_v3_1_BTC.py
# SHA-256：b71dc0c85a5ae3cff458c31c06a364bfb6d1bb522df1ca7a5fe09593aae41c2e
# 完整复制 V3.1 交易体，不继承任何冻结类。本文件两个类：
#   confirm_v3_1_BTC_anchor1 只改 3h 锚点，使 3h 柱在 UTC 01/04/07… 收盘
#                            （pandas resample 180min offset=-2h）。
#   confirm_v3_1_BTC_anchor2 只改 3h 锚点，使 3h 柱在 UTC 02/05/08… 收盘
#                            （offset=-1h）。V3.1 默认锚点 0 收盘于 03/06/09。
# 重采样与合并对齐逻辑逐字复用 confirm_v3_BTC_sigaudit.py（已通过 lookahead；
# verify_resample_align.py 曾验证 n_premature_1h=0）。
# 预注册：仅审计。门槛见 REPORT 1.5(b)。禁止 Hyperopt。无同名 JSON。
# =============================================================================
...
class confirm_v3_1_BTC_anchor1(IStrategy):
    """V3.1 + 3h 锚点偏移，使 3h 柱在 UTC 01/04/07 收盘。仅审计。"""
...
class confirm_v3_1_BTC_anchor2(confirm_v3_1_BTC_anchor1):
    """V3.1 + 3h 锚点偏移，使 3h 柱在 UTC 02/05/08 收盘。仅审计。"""
