...
# isort: skip_file
# =============================================================================
# VolatilitySystemTrailConfirmRegimeEMA200：3h EMA200 方向过滤（非已证实优势）
#
# 复制自冻结快照 confirm_v1_hyperopt（trail_atr_mult=4.5）。
# Confirm V1、冻结 4.5、同名 JSON 与 freqtrade_confirm_fwd 一律不动。
# 3h 收盘高于 EMA200 只做多，低于只做空；第二次确认同样要求趋势过滤后的 enter_*。
# 反向退出固定未过滤的 2.0×ATR。不优化 EMA 周期，不加斜率。
# 仓位、跟踪、止损、300 上限、2x、BTC/ETH 全部冻结。
# =============================================================================
...
class VolatilitySystemTrailConfirmRegimeEMA200(IStrategy):
    """冻结 4.5 规则 + 3h EMA200 方向过滤。反向退出固定 2.0×ATR。"""
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        resampled["regime_ema_200"] = ta.EMA(resampled, timeperiod=200)
        resampled["regime_close"] = resampled["close"]
        ...
        dataframe["regime_ema_200"] = dataframe[
            f"resample_{resample_int}_regime_ema_200"
        ]
        dataframe["regime_close"] = dataframe[
            f"resample_{resample_int}_regime_close"
        ]
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (dataframe["close_change"] > threshold)
            & (dataframe["regime_close"] > dataframe["regime_ema_200"]),
            "enter_long",
        ] = 1
