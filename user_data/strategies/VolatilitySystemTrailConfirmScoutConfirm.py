...
# isort: skip_file
# =============================================================================
# VolatilitySystemTrailConfirmScoutConfirm：弱突破试仓、强突破才加仓（非已证实优势）
#
# 复制自冻结快照 confirm_v1_hyperopt（trail_atr_mult=4.5）。
# Confirm V1、冻结 4.5、同名 JSON 与 freqtrade_confirm_fwd 一律不动。
# 首次 1.6×ATR 突破只开 25% 试仓；后续独立 2.0×ATR 同向突破才补 75%。
# 反向退出固定 2.0×ATR。跟踪、止损、300 上限、2x 全部冻结。不做 Hyperopt。
# 1.6 是训练窗 1.5–1.7 高覆盖区域中点，不是损失函数单点最优的另作解释。
# =============================================================================
...
class VolatilitySystemTrailConfirmScoutConfirm(IStrategy):
    """冻结 4.5 规则 + 1.6 试仓 / 2.0 加仓。反向退出固定 2.0×ATR。"""
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        atr_prev = dataframe["atr_raw"].shift(1)

        scout_long = dataframe["close_change"] > 1.6 * atr_prev
        scout_short = -dataframe["close_change"] > 1.6 * atr_prev
        confirm_long = dataframe["close_change"] > 2.0 * atr_prev
        confirm_short = -dataframe["close_change"] > 2.0 * atr_prev

        dataframe["confirm_long"] = confirm_long.astype(int)
        dataframe["confirm_short"] = confirm_short.astype(int)

        dataframe.loc[
            scout_long, ["enter_long", "enter_tag"]
        ] = (1, "scout_1p6")
        dataframe.loc[
            scout_short, ["enter_short", "enter_tag"]
        ] = (1, "scout_1p6")

        # 强信号仍先开25%，标签用于独立归因。
        dataframe.loc[confirm_long, "enter_tag"] = "core_2p0"
        dataframe.loc[confirm_short, "enter_tag"] = "core_2p0"
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[dataframe["confirm_long"] == 1, "exit_short"] = 1
        dataframe.loc[dataframe["confirm_short"] == 1, "exit_long"] = 1
        return dataframe
    ...
    def adjust_trade_position(
        ...
        **kwargs,
        ...
        if len(dataframe) > 2:
            ...
            signal_name = "confirm_long" if not trade.is_short else "confirm_short"
