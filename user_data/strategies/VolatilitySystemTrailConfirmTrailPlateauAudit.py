...
# isort: skip_file
# =============================================================================
# VolatilitySystemTrailConfirmTrailPlateauAudit：年度2定点参数平台检验
#
# 以冻结件 confirm_v1_hyperopt.py 为基准，只改类名并增加 3.7 / 4.3 / 4.7 / 5.0 子类。
# 4.5 对照使用 confirm_v1_hyperopt，不在本文件重复。
# 无同名 JSON。禁止把年度2最好的点升级为新参数。
# Confirm V1 与 freqtrade_confirm_fwd 保持不动。
# =============================================================================
...
class VolatilitySystemTrailConfirmTrailAuditBase(IStrategy):
    """冻结快照：Confirm V1 + 4.5×ATR 跟踪。禁止再 Hyperopt。"""
...
class VolatilitySystemTrailConfirmTrailAudit37(
    VolatilitySystemTrailConfirmTrailAuditBase
):
    trail_atr_mult = 3.7


class VolatilitySystemTrailConfirmTrailAudit43(
    VolatilitySystemTrailConfirmTrailAuditBase
):
    trail_atr_mult = 4.3


class VolatilitySystemTrailConfirmTrailAudit47(
    VolatilitySystemTrailConfirmTrailAuditBase
):
    trail_atr_mult = 4.7


class VolatilitySystemTrailConfirmTrailAudit50(
    VolatilitySystemTrailConfirmTrailAuditBase
):
    trail_atr_mult = 5.0
