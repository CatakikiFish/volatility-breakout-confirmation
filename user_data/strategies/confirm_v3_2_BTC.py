...
# isort: skip_file
# =============================================================================
# FROZEN — confirm_v3_2_BTC
#
# 交易体 = confirm_v3_1_BTC + trailclose（3h 收盘极值 ATR 跟踪，盘中触发）。
# 不含 tseg 小时剔除。时段线已关闭。
#
# 母本：user_data/strategies/confirm_v3_1_BTC.py
# SHA-256：b71dc0c85a5ae3cff458c31c06a364bfb6d1bb522df1ca7a5fe09593aae41c2e
# 研究件 trailclose：user_data/strategies/confirm_v3_1_BTC_exit2.py
# SHA-256：e6a3352030cb1cb4dc2f17e3061414c880f7704efd8ff438b57b64e1aebb55f8
# 完整复制 V3.1 交易体，不继承任何冻结类。只改跟踪参考极值：
#   custom_data ext_close = 入场后已完成 3h 柱收盘的最高值（多）/ 最低值（空）。
#   更新点：custom_stoploss 读 dataframe["date"] < current_time 的已收盘 1h；
#   完成行 hour % 3 == 2；after_fill 同样更新。
#   首个值 = 入场后第一个完成的 3h 收盘；ext_close 为空时不启用 ATR 跟踪线，
#   仅 0.5R 锁盈与初始止损生效。
#   _trail_stop_price：多 ext_close − 3.6×atr_raw，空 ext_close + 3.6×atr_raw。
#   触发仍为盘中（custom_stoploss 返回值）。0.5R 锁盈、初始止损、noloosen、
#   R 重算、mcap、2R 激活（仍按 max_rate/min_rate / current_rate 极值判定）不变。
#
# BTC 三窗在时段小时集合设计前已被查看；trailclose 的 BTC 三窗在冻结前亦已查看。
# V3.2 相对 V3.1 的 BTC 改进不构成样本外证据。独立支持来自 2026-09-03 第三轮：
# trailclose 晋级候选 + ETH 支持条款通过 + lookahead has_bias=No。
#
# 禁止 Hyperopt、禁止同名 JSON、禁止覆盖任何前向。
# 禁止改 3.6 / 2.0 / 0.5 / 2×ATR / 6% / 3x / 540 / 10 USDT / 25%/75% / 80%。
# =============================================================================
...
class confirm_v3_2_BTC(IStrategy):
    """Confirm V3.2：V3.1 + 3h 收盘极值 ATR 跟踪（盘中触发）。冻结件。"""
    ...
    # 仅当配置文件未定义同名键时生效。研究命令请加载 confirm_v3_2_BTC.overlay.json。
    stake_amount = 540
    max_open_trades = 1
    ...
    def _update_ext_close(self, trade: Trade, dataframe: DataFrame, current_time: datetime) -> None:
        """Running max/min of completed 3h closes after entry. Completing 1h: hour % 3 == 2."""
        closed = dataframe.loc[dataframe["date"] < current_time, ["date", "close"]]
        if closed.empty:
            return
        open_dt = trade.open_date_utc
        if open_dt.tzinfo is None:
            open_dt = open_dt.replace(tzinfo=timezone.utc)
        dts = pd.to_datetime(closed["date"], utc=True)
        hours = dts.dt.hour
        mask = (hours % 3 == 2) & ((dts + pd.Timedelta(hours=1)) > open_dt)
        if not bool(mask.any()):
            return
        closes = closed.loc[mask.to_numpy(), "close"]
        if not trade.is_short:
            ext = float(closes.max())
        else:
            ext = float(closes.min())
        trade.set_custom_data(key="ext_close", value=float(ext))

    def _trail_stop_price(self, trade: Trade, r_dist: float, atr_raw: float) -> float:
        open_rate = float(trade.open_rate)
        ext = trade.get_custom_data(key="ext_close", default=None)
        if not trade.is_short:
            lock_stop = open_rate + self.trail_lock_R * r_dist
            if ext is None:
                return lock_stop
            atr_stop = float(ext) - self.trail_atr_mult * atr_raw
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        if dataframe is None or dataframe.empty:
            return None
        self._update_ext_close(trade, dataframe, current_time)
        atr_raw = float(dataframe.iloc[-1]["atr_raw"])
        if not np.isfinite(atr_raw) or atr_raw <= 0:
            return None
