...
# isort: skip_file
# =============================================================================
# VolatilitySystemTrailConfirmRiskOpt：探索性风险预算 Hyperopt（非已证实优势）
#
# 复制自 VolatilitySystemTrailConfirmTrailRiskCross（4.5 跟踪 × 1% 风险定仓）。
# Confirm V1、冻结 4.5、交叉分支、同名 JSON 与 freqtrade_confirm_fwd 一律不动。
# 只把账户风险预算做成 risk 空间可优化参数（0.6%–1.4%，步进 0.1%）。
# 4.5 跟踪、2R/0.5R、25%/75%、300 上限、2x、BTC/ETH 全部冻结。
# 训练窗口年度1；否决不扩大 0.6–1.4。不搜索出场。
# =============================================================================
...
class VolatilitySystemTrailConfirmRiskOpt(IStrategy):
    """4.5 跟踪 + 只优化账户风险预算。"""
    ...
    max_trade_risk = 0.12
    account_risk_pct = DecimalParameter(
        0.6,
        1.4,
        decimals=1,
        default=1.0,
    ...
    def _target_total_stake(
        ...
        if equity is None:
            return None
        risk_budget = equity * (float(self.account_risk_pct.value) / 100.0)
    ...
    def custom_stake_amount(
        ...
        **kwargs,
        ...
        # 完整目标仓按初始止损亏 account_risk_pct% 权益定尺，首仓仍只下 25%。
