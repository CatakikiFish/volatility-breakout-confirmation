...
# isort: skip_file
# =============================================================================
# VolatilitySystemTrailConfirmEntryOpt：探索性入场强度 Hyperopt（非已证实优势）
#
# 复制自冻结快照 confirm_v1_hyperopt（trail_atr_mult=4.5）。
# Confirm V1、冻结 4.5、同名 JSON 与 freqtrade_confirm_fwd 一律不动。
# 只把首次/确认入场门槛做成 enter 空间可优化参数（1.5–2.5 × atr_raw，步进 0.1）。
# 反向退出固定原来的 2.0×ATR（dataframe["atr"].shift(1)），不优化出场。
# 仓位、跟踪、止损、300 上限、2x、BTC/ETH 全部冻结。否决不扩大范围。
# =============================================================================
...
from freqtrade.strategy import DecimalParameter, IStrategy, stoploss_from_absolute
...
class VolatilitySystemTrailConfirmEntryOpt(IStrategy):
    """冻结 4.5 规则 + 只优化突破强度。反向退出固定 2.0×ATR。"""
    ...
    entry_atr_mult = DecimalParameter(
        1.5,
        2.5,
        decimals=1,
        default=2.0,
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        threshold = (
            float(self.entry_atr_mult.value)
            * dataframe["atr_raw"].shift(1)
        )
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        frozen_threshold = dataframe["atr"].shift(1)

        dataframe.loc[
            dataframe["close_change"] > frozen_threshold,
            "exit_short",
        ] = 1
