...
# isort: skip_file
# =============================================================================
# confirm_v1_hyperopt_3x_btc_trailopt：BTC 专属一维 TrailOpt（非已证实优势）
#
# 复制 confirm_v1_hyperopt_3x_btc，只把 trail_atr_mult 做成 exit 空间可优化参数。
# 搜索 2.0–5.0、步进 0.1，default=4.5，load=False。禁止围绕已见 3.7 缩窄。
# 3x、540 仓、1 槽、25%/75%、2R/0.5R、6% 价格止损和全部信号不变。
# 训练窗 20200415-20240829。验证失败不得改选第二名，也不得缩窄重搜。
# 不改 confirm_v1_hyperopt_3x_btc、主配置、Confirm 前向或 BTC 三臂容器。
#
# Freqtrade 2026.7：回测/Hyperopt 必须叠加 confirm_v1_hyperopt_3x_btc.overlay.json。
# =============================================================================
...
class confirm_v1_hyperopt_3x_btc_trailopt(IStrategy):
    """BTC 540 集中规则 + 仅优化 3h ATR 跟踪倍数。"""
    ...
    def _trail_stop_price(self, trade: Trade, r_dist: float, atr_raw: float) -> float:
        open_rate = float(trade.open_rate)
        trail_mult = float(self.trail_atr_mult.value)
        if not trade.is_short:
