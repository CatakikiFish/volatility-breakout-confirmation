...
# isort: skip_file
# =============================================================================
# RESEARCH — Confirm V3.1 第三信号单位
#
# 母本：user_data/strategies/confirm_v3_1_BTC.py
# SHA-256：b71dc0c85a5ae3cff458c31c06a364bfb6d1bb522df1ca7a5fe09593aae41c2e
# 完整复制 V3.1 交易体，不继承任何冻结类。只改 adjust_trade_position：
#   nr_of_successful_entries < 3；第二次加仓仍为首仓×3；第三次 = 首仓保证金×1。
#   首仓保证金取第一笔已成交入场 Order.stake_amount（amount×price/leverage），
#   不使用 cost/leverage（回测 Order.cost 含手续费）。
# 80% 上限与 noloosen（after_fill 在 entries>=2 已触发）对第三次同样生效。
# 预注册门槛同出场三变体。禁止 Hyperopt。无同名 JSON。
# =============================================================================
...
class confirm_v3_1_BTC_unit3(IStrategy):
    """V3.1 + 第三次独立信号加仓（首仓×1）。门槛见文件头。"""
        ...
        **kwargs,
        ...
        if len(dataframe) > 2:
            ...
            if (
                ...
                and trade.nr_of_successful_entries < 3
                and trade.orders[-1].order_date_utc < prior_date
            ):
                n_ent = int(trade.nr_of_successful_entries or 0)
                if n_ent < 2:
                    ...
                    if not filled:
                        return None
                    scout_stake = float(filled[0].stake_amount)
                    additional_stake = scout_stake * 1
