...
# isort: skip_file
# =============================================================================
# VolatilitySystemTrail：VolatilitySystemSL + 盈利 2R 后的 ATR 跟踪（Freqtrade 2026.7）
#
# 对照基线：VolatilitySystemSL（初始 ATR 止损 + 12% 上限，不跟踪）。
# 本版不改入场、反向退出、2x、加仓、初始止损公式，只多一条规则：
#
#   R = |开仓均价 − 初始止损价|（每笔成交后按新均价写入 custom_data）
#   未到 2R：继续用初始止损
#   达到 2R 后启动跟踪，且一旦启动不再关闭：
#     多头 max(均价 + 0.5R, 持仓最高价 − 3×当前 atr_raw)
#     空头 min(均价 − 0.5R, 持仓最低价 + 3×当前 atr_raw)
#
# 跟踪止损离场原因仍是 trailing_stop_loss；反向信号退出保留。
# =============================================================================
...
class VolatilitySystemTrail(IStrategy):
    """VolatilitySystemSL + 2R 后 3×ATR 跟踪，锁定 0.5R。"""
