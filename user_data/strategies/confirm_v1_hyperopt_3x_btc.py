...
# Tag: confirm_v1_hyperopt_3x_btc
# 风险等额 BTC 单币分支：confirm_v1_hyperopt_3x 交易规则一字不改，只把双币 270×2
# 收成 BTC 540×1。风险预算不变，只改变资产集中度。禁止同时测 15%/85%。
#
# 冻结：leverage()=3.0，stoploss=-0.18，max_trade_risk=0.18（18%/3=6% 价格），
#   trail_atr_mult=4.5，25%/75%，2R/0.5R，ATR 初始止损，固定仓，自动费率，无细节周期。
#
# 资金：max_open_trades=1，stake_amount=540。首次 135，确认加仓 405，完整名义 1620。
#   540×18%=97.2 USDT，与 BTC+ETH 各 270 同时硬止损的总预算相同；名义敞口同为 1620。
#
# 对照基准（confirm_v1_hyperopt_3x 全历史 03-17-48.zip，不传 timerange）：
#   PF 1.527，收益÷账户回撤 12.83，BTC 腿 862 USDT。机械×2≈1725 不是预期。
# 事先 AND：完整自然年 2021–2025 均盈利；全历史 PF≥1.527；最大回撤≤15%；
#   收益÷账户回撤>12.83；p<0.10；无清算、拒单、显著期末强平贡献。
# 通过也只是 BTC 集中候选，须独立前向验证。不改主配置/前向 Dry-run。
#
# Freqtrade 2026.7：回测必须叠加 user_data/confirm_v1_hyperopt_3x_btc.overlay.json。
# =============================================================================
...
class confirm_v1_hyperopt_3x_btc(IStrategy):
    """冻结快照：4.5×ATR + 3x + BTC 540 固定仓。禁止 Hyperopt，禁止改主配置。"""
