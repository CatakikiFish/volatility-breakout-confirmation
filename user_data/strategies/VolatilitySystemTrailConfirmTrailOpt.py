...
# isort: skip_file
# =============================================================================
# VolatilitySystemTrailConfirmTrailOpt：探索性一维 Hyperopt（非已证实优势）
#
# Confirm V1 保持冻结。只把 trail_atr_mult 做成 exit 空间可优化参数（2.0–5.0，步进 0.1）。
# 2R 启动、0.5R 锁盈、初始止损、25%/75%、300 上限、2x 和全部信号不变。
# 训练窗口年度1；年度2 与两年合并只做验证。否决不扩大搜索空间。
# 已冻结快照：user_data/strategies/confirm_v1_hyperopt.py（trail_atr_mult=4.5 硬编码）。
# =============================================================================
...
class VolatilitySystemTrailConfirmTrailOpt(IStrategy):
    """Confirm V1 规则 + 仅优化 3h ATR 跟踪倍数。"""
    ...
    trail_lock_R = 0.5
    # 唯一可优化参数：跟踪距离 = trail_atr_mult × 当前 3h atr_raw。
    trail_atr_mult = DecimalParameter(2.0, 5.0, default=3.0, decimals=1, space="exit")
