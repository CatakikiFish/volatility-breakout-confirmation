...
# isort: skip_file
# =============================================================================
# VolatilitySystemTrailConfirmLev3：3x 杠杆梯度（保持 6% 价格止损距离；非已证实优势）
#
# 完整复制自 VolatilitySystemTrailConfirmLev25。信号、4.5×ATR 跟踪、300 仓、25%/75% 不变。
# 只改三项：leverage()=3.0，max_trade_risk=0.18，stoploss=-0.18。
# 18%/3 = 6% 价格波动，与 2x 的 12%/2.0、2.5x 的 15%/2.5 相同。
# 插在已通过的 2.5x 与身份失败的 5x 之间。Confirm V1 与前向 Dry-run 保持 2x。
# =============================================================================
...
class VolatilitySystemTrailConfirmLev3(IStrategy):
    """冻结 4.5 规则 + 3x。账户风险上限改为 18%，保持 6% 价格止损距离。"""
