...
# isort: skip_file
# =============================================================================
# VolatilitySystemTrailConfirmStopAudit：初始止损宽度 1.5 / 2.0 / 2.5 对照（非已证实优势）
#
# 复制自冻结快照 confirm_v1_hyperopt。基类交易逻辑一字不改。
# Confirm V1、冻结 4.5、同名 JSON 与 freqtrade_confirm_fwd 一律不动。
# 只改子类 stoploss_atr_mult；会同时改变 R、2R 启动与 0.5R 锁盈。
# max_trade_risk=0.12 仍是 2x 下的价格距离上限。不做 Hyperopt。
# =============================================================================
...
class VolatilitySystemTrailConfirmStopAuditBase(IStrategy):
    """冻结 4.5 规则基类。子类只覆盖 stoploss_atr_mult。"""
...
class VolatilitySystemTrailConfirmStop15(
    VolatilitySystemTrailConfirmStopAuditBase
):
    stoploss_atr_mult = 1.5


class VolatilitySystemTrailConfirmStop25(
    VolatilitySystemTrailConfirmStopAuditBase
):
    stoploss_atr_mult = 2.5
