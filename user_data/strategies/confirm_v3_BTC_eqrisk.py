...
# isort: skip_file
# =============================================================================
# RESEARCH — confirm_v3_BTC_eqrisk（单变量：风险预算随权益复利）
#
# 母本：confirm_v3_BTC.py
# SHA-256：9066d4172499494ed2d09a9dff896cd7c967dce2a555c72432b9a4f746111868
# 完整复制 V3 交易体，不继承冻结类。只改一件事：首仓风险预算从固定 10.0 改为
# 0.01 × 当前总权益（self.wallets.get_total_stake_amount()），首仓上限从固定
# proposed_stake/2（270）改为 0.27 × 当前总权益。其余（[min_stake, 上限] 夹取、
# 加仓 ×3、min(·, max_stake)）不变。
...
class confirm_v3_BTC_eqrisk(IStrategy):
    """V3 + 首仓风险预算随权益 1% 复利。单变量。"""
    ...
    def custom_stake_amount(
        ...
        **kwargs,
        ...
        risk_abs = 0.01 * equity
        stake_cap = 0.27 * equity
        stake = risk_abs / (d * leverage)
        stake = min(stake, stake_cap, max_stake)
